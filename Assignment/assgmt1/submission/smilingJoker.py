...
def transform_input(x):
    ...
    phi = x.copy()
    phi = np.append(phi, x**2, axis=1)
    phi = np.append(phi, np.cos(x), axis=1)
    return phi
...
def read_dataset(filepath):
    ...
    n = len(dataset)
    n_dash = int(0.9*n)
    X_train = dataset["x"][:n_dash].to_numpy().reshape((-1, 1))
    y_train = dataset["y"][:n_dash].to_numpy().reshape((-1, 1))
    X_test = dataset["x"][n_dash:].to_numpy().reshape((-1, 1))
    y_test = dataset["y"][n_dash:].to_numpy().reshape((-1, 1))
